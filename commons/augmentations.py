...
class RandScaleMinMax(BasicTransform):
    def __init__(self, min_thresh, max_thresh=1024, **kwargs):
        kwargs['p'] = 1.0
    ...
    def aug(self, box_info: BoxInfo) -> BoxInfo:
        img, ratio = self.scale_img(box_info.img, self.min_thresh)
        box_info.img = img
        if box_info.boxes is not None and len(box_info.boxes):
...
class Mosaic(BasicTransform):
    ...
    def aug(self, box_info: BoxInfo) -> BoxInfo:
        ...
        if len(box_info4):
            ...
            return self.affine(original_box_info)
...
class BatchPadding(object):
    def __init__(self,
                 ...
                 padding_value=(103, 116, 123),
                 center_padding=False,
                 size_divisible=32):
        """
        :param rgb_mean:
        :param rgb_std:
        :param padding_value: (bgr)
        """
    ...
    def __call__(self, items):
        """
        :param items(list):  list(item)
                             item.img:  ("bgr" channel order, unnormalized)
                             item.boxes(np.array): shape=[n,4]  4==>x1,y1,x2,y2  unnormalied
                             item.labels(np.array): shape=[n,]  label_id
        :return:
        ret_imgs [batch_size,h,w,3]
        ret_labels [gt_nums,6]  6==>batch_id,label_id,x1,y1,x2,y2(unnormalized)
        """
        # imgs(list): img ("bgr" channel order)
        imgs = [item.img for item in items]
        # labels(list): [gt_num,5] (label_idx,x1,y1,x2,y2)
        labels = [np.concatenate([item.labels[:, None], item.boxes], axis=-1) for item in items]

        assert len(imgs) == len(labels), "len(imgs) should be equal to len(labels)"
        hw_shapes = np.array([(item.shape[:2]) for item in imgs])
        ...
        for i, (img, label) in enumerate(zip(imgs, labels)):
            ...
            ret_img = cv.copyMakeBorder(img, top, bottom, left, right, cv.BORDER_CONSTANT, value=self.padding_value)
            if len(label):
                # [batch_idx,label_idx,x1,y1,x2,y2]
                temp_label = np.empty(shape=(label.shape[0], label.shape[1] + 1))
                temp_label[..., 0] = i
                temp_label[..., [1]] = label[..., [0]]
                temp_label[..., [2, 4]] = (label[..., [1, 3]] + left)
                temp_label[..., [3, 5]] = (label[..., [2, 4]] + top)
                # import uuid
                # from commons.boxs_utils import draw_box
                # from datasets.coco import colors, coco_names
                # temp_label[..., [2, 4]] = temp_label[..., [2, 4]]
                # temp_label[..., [3, 5]] = temp_label[..., [3, 5]]
                # draw_img = draw_box(ret_img, temp_label[..., 1:], colors, coco_names)
                # cv.imwrite("../datasets/{:d}_{:s}.jpg".format(i, str(uuid.uuid4()).replace('-', "")), draw_img)
                ret_labels.append(temp_label)
            if self.rgb_mean is not None and self.rgb_std is not None:
                ret_img = ((ret_img[..., ::-1].astype(np.float32) / 255.0) - np.array(self.rgb_mean)) / np.array(self.rgb_std)
            ret_imgs.append(ret_img)
        ret_imgs = np.stack(ret_imgs, axis=0)
        ret_labels = np.concatenate(ret_labels, axis=0)
        batch_shapes = np.array(batch_shapes)
        return ret_imgs, batch_shapes, ret_labels
